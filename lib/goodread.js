        ...
        return new Promise((resolve) =>{
            ...
            grclient.searchBooks(query_book).then(response => {
              ...
              let search = response.search,
                  ...
                  rating_count,
                  count = 0;
                  console.log(search.results.work);
              // Check how many results come back. JSON response format changes based on number of results(books author has).
              if(parseInt(search['total-results']) == 1){
                if(typeof  search.results.work.average_rating == "object"){
                  rating = search.results.work.average_rating._;
                }else{
                  rating = search.results.work.average_rating;
                }                   
                bookId = search.results.work.best_book.id._;
                if(typeof search.results.work.ratings_count =="object"){
                  rating_count = search.results.work.ratings_count._;
                }else{
                  rating_count = search.results.work.ratings_count
                }
              }else if(parseInt(search['total-results']) > 1){
                    ...
                    if(typeof search.results.work[randomNr].ratings_count == "object"){
                      rating_count = search.results.work[randomNr].ratings_count._;
                    }else{
                      rating_count = search.results.work[randomNr].ratings_count;
                    }                    
                    ...
                    }while(rating <= 3.8  && rating_count <= 5)
                ...
                if((rating < 3.8 && parseInt(rating_count) <= 5) || rating < 3.8 || parseInt(rating_count) <= 5){
