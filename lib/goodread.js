    ...
    getBook : function(word){
        ...
        return new Promise((resolve) =>{
            ...
            grclient.searchBooks(query_book).then(response => {
                ...
                if(typeof results.work =='undefined' ||response["total-end"]>0){
                        logger.log(logger.error,'Results.work: '+JSON.stringify(results.work));
                        setTimeout(
                            function(){
                    ...
                    let rating,
                        bookId;
                    logger.log(logger.info,'Results.work[nr]:' + typeof  results.work[randomNrB])
                    if(typeof results.work[randomNrB] === "undefined" && results.work.average_rating._ < 3.8){
                        ...
                        // If it's heigher than 3.8 and only one book is fine continue
                        ...
                        // Grab the book id when all is finished
                    ...
                    grclient.showBook(bookId).then(response => {
                        ...
                        if(lang =='eng' || lang=='en-US' || lang=='en-GB' || lang=='English' || lang=='english' || lang=='en-US'){
                            ...
                            logger.log(logger.error,'Language not english. Restarting process');
                            setTimeout(
                                function(){
                                    logger.log(logger.warn,'Timeout ended. Restart after 1s');
                                    resolve(self.getBook(self.word()));
